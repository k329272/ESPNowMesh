...
void ESPNowMesh::handleDiscoveryProbe(const uint8_t* senderMAC, const MeshMessage* msg) {
  ...
  if (_configSyncEnabled && msg && msg->payloadSize >= sizeof(MeshConfig)) {
...
void ESPNowMesh::setMeshConfig(const MeshConfig& config, bool broadcast) {
  _meshConfig = config;
  if (_meshConfig.configVersion == 0) {
    _meshConfig.configVersion = 1;
  }
...
void ESPNowMesh::handleConfigAdvertisement(const uint8_t* senderMAC, const MeshMessage* msg) {
  if (!_configSyncEnabled || !msg || msg->payloadSize < sizeof(MeshConfig)) return;
...
void ESPNowMesh::handleConfigRequest(const uint8_t* senderMAC, const MeshMessage* msg) {
  (void)msg;
  if (!_configSyncEnabled || compareMac(senderMAC, myMAC)) return;
...
void ESPNowMesh::applyMeshConfig(const MeshConfig& config, const uint8_t* sourceMac, bool forceApply) {
  ...
  uint32_t incomingSignature = meshConfigSignature(normalized);
  bool isNewer = config.configVersion > _meshConfig.configVersion;
  ...
  if (!forceApply && _configSynced && !isNewer) {
    return;
  }
